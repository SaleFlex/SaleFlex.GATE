...
- **POS-aligned domain models** — `Cashier` (custom `AUTH_USER_MODEL`: universal GATE user + POS profile fields), merchant, store, POS, closure, warehouse, customer, and related reference entities under `pos_api_app` (evolving toward full sync with PyPOS payloads).
...
- **Web UI & public portal** — `web_ui_app` provides a landing page (guests), session login/register/logout, **portal-only** change password (`/accounts/password/change/`, templates under `web_ui_app/templates/web_ui_app/`), a signed-in **left sidebar** (dashboard, **companies**, profile, password, plus “coming soon” placeholders), **company creation** and **join-by-slug requests** with **owner tag** vs **administrator** rules, multi-owner **deletion approvals**, a dashboard stub, a **site header** with the product brand and an **account menu** only (avatar with default placeholder, profile edit, settings submenu with change password), and optional profile picture uploads. **Company** records support optional registration fields (`companies_house_number`, `vat_number`, `registered_office`) suited to typical **UK limited company** data; only the **company name** is required at creation, and owners or administrators can edit details on the company page (slug is fixed). ERP-style screens will extend this.
...
- **Data layer** — All Django ORM models live in a dedicated `core` app backed by SQLite (dev) or PostgreSQL (production). `pos_api_app` and `web_ui_app` contain zero model definitions — they import from `core.models`. Concrete core models inherit common audit fields (`created_by`, `updated_by`, `created_at`, `updated_at`) from `core.models.BaseModel`. `Cashier` is the custom user (`AUTH_USER_MODEL`) and shared identity for GATE portal, OFFICE, PyPOS, and mPOS; **per-company** roles live on `CompanyMembership`; `CashierStoreAssignment` records per-store POS device access; portal models (Company, membership, join/deletion) and POS domain models (Merchant, Store, PointOfSale, Closure, Warehouse, …) all live under `core/models/`.
...
│   ├── settings.py           # INSTALLED_APPS, AUTH_USER_MODEL, REST_FRAMEWORK, DATABASES
│   ├── urls.py               # Root URLconf (admin, future API includes)
│   ├── wsgi.py
│   └── asgi.py
│
├── core/                     # ★ Data layer — all models live here
│   ├── admin.py              # All model admin registrations (Cashier, Company, …)
│   ├── apps.py               # CoreConfig (verbose_name = "SaleFlex Core")
│   ├── migrations/           # Single migration source for all core models
│   └── models/               # One file per model; all re-exported from __init__.py
│       ├── __init__.py       # Full export of every core model
│       ├── base.py           # BaseModel — shared audit/timestamp fields
│       ├── cashier.py        # Cashier — AUTH_USER_MODEL (avatar, PIN, cashier_number, soft-delete)
│       ├── cashier_store_assignment.py  # CashierStoreAssignment — per-store POS device access
│       ├── company.py        # Company
...
│       ├── pos.py            # PointOfSale
│       ├── closure.py        # Closure
│       └── …                 # Warehouse, Customer, reference tables, etc.
│
├── pos_api_app/              # POS / merchant API layer — NO model definitions
│   ├── authentication/       # MerchantTokenAuthentication
│   ├── models/
│   │   └── __init__.py       # Shim: re-exports from core.models for backward compat
│   ├── admin.py              # (empty — admin owned by core)
│   ├── apps.py
│   └── tests.py
│
├── web_ui_app/               # Portal UI — NO model definitions
│   ├── forms.py              # Registration, avatar, company forms
│   ├── context_processors.py # cashier → signed-in Cashier (AUTH_USER_MODEL)
│   ├── urls.py               # /, /dashboard/, /accounts/...
│   ├── static/web_ui_app/    # Portal CSS, favicon, optional JS
│   │   ├── css/base.css
│   │   ├── icons/favicon.svg
│   │   ├── icons/avatar-default.svg
│   │   └── js/
│   ├── templates/            # Landing, login/register, profile, password_change
│   ├── admin.py              # (empty — admin owned by core)
│   ├── apps.py
│   ├── models/
│   │   └── __init__.py       # Shim: re-exports from core.models for backward compat
│   ├── views/                # One module per portal view
│   └── tests.py
│
...
- **`AUTH_USER_MODEL`** — Set to `core.Cashier` (custom user). See [docs/11-gate-user-model.md](docs/11-gate-user-model.md).
...
1. **Cashier account** (`AUTH_USER_MODEL`)  
...
| [docs/10-shared-models-package.md](docs/10-shared-models-package.md) | Legacy `web_ui_app.models` shim notes; current model ownership is in `core.models`. |
| [docs/11-gate-user-model.md](docs/11-gate-user-model.md) | `Cashier` (`AUTH_USER_MODEL`), `CompanyMembership` per-company roles, and `CashierStoreAssignment` — identity across GATE, OFFICE, PyPOS, mPOS. |
| [docs/12-core-app.md](docs/12-core-app.md) | `core` app model ownership, `BaseModel`, import conventions, and migration guidance. |
