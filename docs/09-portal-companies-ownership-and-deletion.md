...
- **Portal company models** — `Company`, `CompanyMembership`, `CompanyJoinRequest`, `CompanyDeletionRequest`, and `CompanyDeletionApproval` live in `core/models/` and are imported via `core.models`. They are the **browser session** source of truth for organization membership until the same rules are mirrored on REST APIs and optionally linked to `Merchant`.
- **Hub user (`Cashier`)** — Authentication and shared identity (avatar, POS PIN, cashier number) use `core.models.Cashier` (`AUTH_USER_MODEL`). **Per-company** roles (owner, admin, store manager, POS cashier, OFFICE) are on `CompanyMembership`, not on the user row. See [11-gate-user-model.md](11-gate-user-model.md).
...
**Per-company POS / OFFICE / store roles:** `CompanyMembership` includes `is_store_manager`, `is_pos_cashier`, and `is_office_user`. They are scoped **per company** (the same `Cashier` can differ by tenant). The portal UI currently centres on owner vs administrator vs member; these booleans support RBAC in the data model and downstream clients. See [11-gate-user-model.md](11-gate-user-model.md).
...
The user who creates a company receives **`is_owner=True`**, **`is_admin=True`**, and **`is_office_user=True`** on `CompanyMembership` (creator can use OFFICE for that company’s data).

#### Optional registration fields (UK limited company shape)

`core.models.Company` stores **optional** identifiers named without a country prefix; they are intended to match typical **UK limited company** filings:
...
- Models: `core/models/` package — one file per model class, all re-exported from `core/models/__init__.py`:
  - `core/models/company.py` — `Company`
  - `core/models/company_membership.py` — `CompanyMembership`
  - `core/models/company_join_request.py` — `CompanyJoinRequest`
  - `core/models/company_deletion.py` — `CompanyDeletionRequest`, `CompanyDeletionApproval`
  - `core/models/base.py` — `BaseModel` audit fields used by these models.
